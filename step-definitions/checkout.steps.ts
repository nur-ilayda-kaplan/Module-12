...
Given('the user is on the product page for {string}', async function (product: string) {
...
When('the user proceeds to checkout and fills payment details', async function () {
  const { config } = await import('../support/config');
  const cart = new CartPage(this.page);
  await cart.checkout();
